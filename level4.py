...
class Level4:
    pygame.init()
    bg = pygame.image.load('Level4Background.png')
    ...
    def __init__(self, win, love, score, mute):
        ...
        self.touch = [False, False, False, False, False, False]
        self.mute = mute
        self.walkCount = 0
        self.angle = 0
        self.time = 0
        ...
        self.timeleft = 0
        self.score = score
        self.bomber = BOMB(173, 335, 5, (255, 255, 255))
    ...
    def redrawWindow(self):
        ...
        if self.wave < 5:
            ...
            text = Level4.font.render('LEVEL ' + str(self.level) + ' / ' + 'WAVE 5', 1, (255, 255, 255))
    ...
    def RUN(self):
        while self.run:
                ...
                if self.shoot:
                    if self.bomber.y < 540 - self.bomber.radius:
                        ...
                        if self.bomber.y > 530 - self.bomber.radius \
                                and self.bomber.y < 550 - self.bomber.radius:
                            self.bomber.land()
                            self.bomber.explo()
            ...
            if self.wave == 5:
                ...
                if self.block5.health <= 0 and not self.block5.spirit():
                    for i in range(1, len(self.enermys)):
                        if self.enermys[i].x >= self.block1.x:
                            self.enermys[i].left()
                            self.enermys[i].walk(self.win)
                        if self.enermys[i].health > 0 and self.enermys[i].x <= self.block1.x + 20 and not self.touch[
                            i - 1]:
                            self.love -= 1
                            self.touch[i - 1] = True
